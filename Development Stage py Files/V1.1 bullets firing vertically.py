...
keys = [False, False, False] #move right and left in the same order
...
##                print ([math.atan2(position[1]-(playerpos1[1]),position[0]-(playerpos1[0])),playerpos1[0]+60,playerpos1[1]+32]) 
        ...
        if keys[2]:
            bb+=1
            if bb == (FPS/5): #60 FPS so will fire 5 times in 1 second
                shoot.play()
##                position=pygame.mouse.get_pos()
                acc[1]+=1
                arrows.append([math.atan2((position[1]-100)-playerpos1[1],0),playerpos1[0]+60,playerpos1[1]+40])
                bb = 0
